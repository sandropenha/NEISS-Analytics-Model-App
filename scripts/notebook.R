...
The data was collected by National Eletronic Injury Surveillance ([https://www.cpsc.gov/Research--Statistics/NEISS-Injury-Data)](https://www.cpsc.gov/Research–Statistics/NEISS-Injury-Data)))
...
-   Step 1: Analysis
    ...
    In this step we gonna do the data wrangling process and also data exploratory analysis.

-   Step 2: Predctive model

    In this step, we gonna construct a machine learning model to predict the number of injuries that will occur for the next 12 months. (in this case, all 2022 year).

-   Ste 3: Web App / Deploy

    In this step, we gonna make a web app for analytics and deploy our machine learning model.
...
First of all, let's create a project directory and then separate our scripts, raw and processed data in differents folders. That will help to keep a more organized (and even effective) workflow.
...
After load and set the work directory, it's time to start the data wrangling process. This step is fundamental to actually know the data, visualize some patterns, and make some treatments.
...
Apparently, looks like the dataset has sort of "label-encoding". We will need to check the data dictonary ("<https://www.cpsc.gov/Research--Statistics/NEISS-Injury-Data>"). This is step is not mandatory, but in this case we gonna replace all the label-encoding for its original values. That will allows us to do the analytical process on a more easy legible way. We might need to check the NEISS code manual too
...
its very common, perhaps it indicates type written mistake. as we aren't certain about those, we'll just discard them.
...
These variables we'll just discard from the dataset:
...
As we can see, the variables Product_1 still having NA values. As we dont know what kind of injuries could replace those, let's just drop them ou of the dataset. We'll alsorename the columns, and then save the new dataset.
...
we'll gonna back to this later when we'll use machine learning to predict the number of injuries. By now, lets focus on analyze the the data.
...
Now that we identified the top 5, lets see more deeply at the number one patterns.

Lets also see Diagnosis, Location e body_parts frequencies.
...
Internal Organ injury is what more appears in the top injuries considering injuries by Floor and Flooring Materials type.
...
Now lets visualize the distribution by sex and age of people who suffered this type of injurie.

but first, we'll gonna look the age variable if it has some incosistancy (which is very usual in this kind of variable)

```{r}
df |> select(Age) |> arrange(desc(Age)) |> unique()
```

Looks like the variable age has some trobles... lets desconsider age \> 85
...
Thats intersting.. apparently kids and seniors suffer more injuries by floor and flooring materials. Thats tottaly makes senses,but that is a important thing to say: This dataset is about people who went to the hospital and it's not about the populatin in general.
...
To do this right, lets use a population dataset of USA by age and sex
...
A reminder: The "Disposition" variable indicates wich treatment the pacient get.
...
And then, studying the data, wich follows:
...
**Seasonality:** Are periodic floatings phenomena that repeats on each period. For example, panettone sales wich grows significantly on december.
...
Model uses both an integrated autoregressive model and a moving average model. Includes Autoregressive (AR) variable, that is, it uses the target variable itself to project it from the variable in the past. It also presents Moving Average (MA), since it uses previous errors to make the forecast;
...
On this comparison, the best model is Holt Winters,
...
- Unfortunately, the Ljung-Box test indicates a p-value below 0.05. It not necessary bad, but how closely p-value is from 0.05 more stable the model is. Whatever, we gonna keep this model because the others premisses looks ok.
